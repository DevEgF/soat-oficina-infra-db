...
foreach ($required in @('DESTROY-soat-oficina-db', 'create-db-snapshot', 'db-snapshot-available', 'disable-protection.tfplan', 'destroy.tfplan', 'snapshot_arn')) {
...
if ($snapshotIndex -lt 0 -or $disableProtectionIndex -lt 0 -or $snapshotIndex -gt $disableProtectionIndex) {
    throw 'final snapshot must be created before deletion protection is disabled'
}
